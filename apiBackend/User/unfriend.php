...
if ($successResponse){
	$stmt = $authPDO->prepare("SELECT * FROM friendreq WHERE requester = :followed AND requested = :userid AND accepted = 1");
	...
	if ($didUserRequest !== 0){
		$stmt = $authPDO->prepare("DELETE FROM friendreq WHERE requester = :followed AND requested = :userid");
		$stmt->bindParam(":userid", $userId, PDO::PARAM_INT);
		$stmt->bindParam(":followed", $followedUserId, PDO::PARAM_INT);
		$stmt->execute();
