...
if false then
 delay(0, function()
   ...
   l.Image = "http://www.<?= $domain ?>/asset/?id=23573247"
...
pcall(function() game:SetPlaceID(<?= $server["PlaceId"] ?>, <?= (string)$server["IsRobloxPlace"] ?>) end)
...
function reportContentProvider(time, queueLength, blocking)
	pcall(function()
		game:HttpGet("http://www.<?= $domain ?>/Analytics/ContentProvider.ashx?t=" .. time .. "&ql=" .. queueLength, blocking)
	end)
end
function reportCdn(blocking)
	pcall(function()
		...
		if successDelta > 0 or failureDelta > 0 then
			game:HttpGet("http://www.<?= $domain ?>z/Game/Cdn.ashx?source=client&success=" .. successDelta .. "&failure=" .. failureDelta, blocking)
		end
	end)
end
...
function reportDuration(category, result, duration, blocking,errorType)
	...
	pcall(function() game:HttpGet("http://www.<?= $domain ?>/Game/JoinRate.ashx?st=0&i=0&p=-1&c=" .. category .. "&r=" .. result .. "&d=" .. (math.floor(duration*1000)) .. "&ip=localhost&errorType=" .. errorType .. "&platform=" .. platform, blocking) end)
end
-- arguments ---------------------------------------
...
pcall(function() game:GetService("GamePassService"):SetPlayerHasPassUrl("http://www.<?= $domain ?>/Game/GamePass/GamePassHandler.ashx?Action=HasPass&UserID=%d&PassID=%d") end)
pcall(function() game:SetCreatorID(<?= AssetService::getAssetInfo($server["PlaceId"])["creatorId"] ?>, Enum.CreatorType.User) end)

-- Bubble chat.  This is all-encapsulated to allow us to turn it off with a config setting
pcall(function() game:GetService("Players"):SetChatStyle(Enum.ChatStyle.ClassicAndBubble) end)

local waitingForCharacter = false
local waitingForCharacterGuid = "8b2f9e4f-d1a4-4284-9b07-25dcb72bedfd";
pcall( function()
...
local success, err = pcall(function()	
	...
	playerConnectSucces, player = pcall(function() return client:PlayerConnect(<?= $user["Id"] ?>, <?= $server["MachineAddress"] ?>, <?= $server["ServerPort"] ?>, 0, threadSleepTime) end)
	if not playerConnectSucces then
		...
		client:Connect(<?= $server["MachineAddress"] ?>, <?= $server["ServerPort"] ?>, 0, threadSleepTime)
	else
		analytics("Created Player")
	end
	...
	player:SetSuperSafeChat(<?= (string)$user["SuperSafeChat"] ?>)
	pcall(function() player:SetMembershipType(Enum.MembershipType.<?= $server["MembershipType"] ?>) end)
	pcall(function() player:SetAccountAge(<?= $user["Joined"] ?>) end)
	player.Idled:connect(onPlayerIdled)
	
	-- Overriden
	onPlayerAdded(player)
	
	pcall(function() player.Name = [========[<?= $user["Name"] ?>]========] end)
	player.CharacterAppearance = "http://www.<?= $domain ?>/Asset/CharacterFetch.ashx?placeId=<?= $server["PlaceId"] ?>&userId=<?= $user["Id"] ?>"	
	if not test then visit:SetUploadUrl("")end
...
pcall(function() game:SetVideoInfo('<?xml version="1.0"?><entry xmlns="http://www.w3.org/2005/Atom" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://gdata.youtube.com/schemas/2007"><media:group><media:title type="plain"><![CDATA[ROBLOX Place]]></media:title><media:description type="plain"><![CDATA[ For more games visit http://www.idk18.xyz]]></media:description><media:category scheme="http://gdata.youtube.com/schemas/2007/categories.cat">Games</media:category><media:keywords>ROBLOX, video, free game, online virtual world</media:keywords></media:group></entry>') end)
